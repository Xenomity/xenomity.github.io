...
구글 STADIA와 같이, '언제 어디서나 어떤 기기에서든' 게임을 즐길 수 있는 게임 환경을 목표로 서비스 구축 내용 공유.
...
- 스트리밍을 위한 클라우드를 구축하고, 리소스를 더 투자해서 게임과 스트리밍, 커뮤니티, 콘텐츠라는 연결고리를 만들어갈 것.
...
- 일반적인 게임 플레이의 상호 작용 단계.
    ...
    - 손이 반응해 콘트롤러에 명령을 내리고 그 명령이 하드웨어(디코더)에 도달하는 전달 과정.
